 ...
 * @version     SVN: $Id: shadowbox.js 99 2008-05-11 16:22:43Z mjijackson $
 */
...
(function(){
    ...
    /**
     * You're not sill using IE6 are you? People like you make my job difficult.
     *
    ...
    var animate = function(el, p, to, d, cb){
        ...
        var fn = function(ease){
            SL.setStyle(el, p, from + ease * (to - from) + unit);
        };
    ...
    var getPlayer = function(url){
        ...
        if(RE.wmp.test(url)){
            if(plugins.wmp){
                ...
                return client.isMac ? (plugins.qt ? 'unsupported-f4m' : 'unsupported-qtf4m') : 'unsupported-wmp';
            }
    ...
    var hideBars = function(anim, cb){
        ...
        // hide both bars after margin is set
        var fn = function(){
            // build the title
            var t = obj.title || '';
            title_i.innerHTML = t;
            /*
            var th = getComputedHeight(title_i);
            var h = (t == '' ? 0 : th);
            if(h) SL.setStyle(title_i, 'top', th + 'px');
            // adjust title height, animate to prevent jerky movement
            animate(title, 'height', h, 0.35, function(){
                SL.setStyle(title_i, 'top',  h + 'px');
            });
            */
            ...
            if(nav){
                ...
                if(options.displayNav){
                    ...
                    // in a slideshow?
                    /*
                    if timer === null then pause or open was just called
                    */
                    // if timer === null then pause was called explicitly, so if
                    // it's not null and there is a delay, show the pause symbol
                    var next = gallery.length > 1 && (current != gallery.length - 1 || options.continuous);
                    if(options.slideshowDelay > 0 && next){
                        pa = slide_timer !== null;
                        pl = !pa;
                    }
                }else{
            ...
            var h = getComputedHeight(info_i);
            SL.setStyle(info_i, 'bottom',  h + 'px');
            // adjust info bar height
            SL.setStyle(info, 'height', h + 'px');
        ...
        if(anim && options.animate){
            // animate the transition
            animate(title_i, 'top', title_h, 0.35);
            animate(info_i, 'top', info_h, 0.35, fn);
        }else{
            SL.setStyle(title_i, 'top', title_h + 'px');
            SL.setStyle(info_i, 'top', info_h + 'px');
            fn();
        }
    };
    ...
    var showBars = function(cb){
        ...
        if(options.animate){
            if(t) animate(title_i, 'top', 0, 0.35);
            animate(info_i, 'top', 0, 0.35, cb);
        }else{
            if(t) SL.setStyle(title_i, 'top', '0px');
            SL.setStyle(info_i, 'top', '0px');
            cb();
        }
    };
    ...
    var loadContent = function(){
        ...
        switch(obj.type){
            case 'msg':
                obj.content = '<div class="shadowbox_message">' + obj.content + '</div>';
                // fall through
            case 'inline':
                t = 'html';
            break;
        ...
        // make sure player for given content type is loaded
        if(typeof SB[t] != 'function'){
            SB.raise('Cannot load player for content type ' + obj.type);
        }
        content = new SB[t](content_id, obj); // instantiate new content object
                ...
                resizeContent(function(){
                    ...
                    showBars(function(){
                        ...
                        toggleLoading(false, function(){
                            ...
                            if(slide_timer !== null && options.slideshowDelay > 0 &&
    ...
    var setDimensions = function(height, width, resizable){
        ...
        if(resizable && (handle == 'resize' || handle == 'drag')){
            ...
            if(handle == 'resize'){
                ...
                if(link) drag = link.type == 'img' && (change_h > 0 || change_w > 0);
            }
        }
    ...
    var resizeContent = function(cb){
        ...
        if(cb){
            ...
            if(c){
                ...
                if(gallery[current].type == 'img' && options.handleOversize == 'drag'){
    ...
    /**
     * Sets the size and position of the container element. This is only
     * necessary for IE6 where the container uses absolute positioning instead
     * of fixed.
     *
     * @return  void
     * @private
    ...
    var resizeContainer = function(){
        SL.setStyle(SL.get('shadowbox_container'), {
            top:    document.documentElement.scrollTop + 'px',
            height: SL.getViewportHeight() + 'px',
            width:  SL.getViewportWidth() + 'px'
        });
    };
    ...
    var toggleVisible = function(on){
        ...
        if(on){
            ...
            if(ltIE7){
                resizeContainer(); // resize before showing
                SL.addEvent(window, 'scroll', resizeContainer);
            }
        }else{
            SL.removeEvent(so, 'click', SB.close);
            if(ltIE7) SL.removeEvent(window, 'scroll', resizeContainer);
        }
        SL.get('shadowbox_container').style.visibility = on ? 'visible' : 'hidden';
    };
    ...
    Shadowbox.init = function(opts){
        ...
        if(ltIE7){
            ...
            SL.setStyle(SL.get('shadowbox_body'), 'zoom', 1);
            ...
            if(png && png.constructor == Array){
                for(var i = 0; i < png.length; ++i){
                    var el = SL.get(png[i]);
                    if(el){
                        var bg = SL.getStyle(el, 'background-image');
                        var match = bg.match(/url\("(.*)"\)/);
                        if(match && (/\.png$/.test(match[1]))){
                            SL.setStyle(el, {
                                'background-image': 'none',
                                'filter': 'progid:DXImageTransform.Microsoft.AlphaImageLoader(src="' + match[1] + '",sizingMethod="scale")'
                            });
                        }
                    }
                }
    ...
    /**
     * Loads the specified skin for use with Shadowbox. If the skin is
     * included already in the page, this function does not need to be
     * called. Otherwise, this function must be called before window.onload.
     * If no skin is given, the classic skin will be used.
     *
     * @param   {String}    skin        The skin directory
     * @return  void
     * @public
     * @static
     */
    Shadowbox.loadSkin = function(skin){
        if(!skin) skin = 'src/skin/classic/';
        if(!(/\/$/.test(skin))) skin += '/';

        // use document.write so this will load before window.onload
        document.write('<link rel="stylesheet" type="text/css" href="' + skin + 'skin.css">');
        document.write('<scr' + 'ipt type="text/javascript" src="' + skin + 'skin.js"><\/script>');
    };

    /**
     * Loads the specified language file to be used with Shadowbox.
     *
     * @param   {String}    lang        The language abbreviation (e.g. en)
     * @param   {String}    dir         (optional) The directory where the
     *                                  Shadowbox language files are kept
     * @return  void
     * @public
     * @static
     */
    ...
    Shadowbox.open = function(obj, opts){
        ...
        // are any media in the gallery supported?
        var match, r;
        for(var i = 0, len = gallery.length; i < len; ++i){
            r = false;
            if(gallery[i].type == 'unsupported'){ // don't support this at all
                r = true;
            }else if(match = RE.unsupported.exec(gallery[i].type)){ // handle unsupported elements
                if(options.handleUnsupported == 'link'){
                    ...
                    gallery[i].content = SB.LANG.errors[s].replace(/\{(\d+)\}/g, function(m, i){
                        return a[i];
                    });
                }else{
                    r = true;
                }
            }else if(gallery[i].type == 'inline'){ // handle inline elements
            ...
            if(r){
                ...
                gallery.splice(i, 1);
                if(i < current) --current; // what if current is unsupported?
                --i;
            }
        }
        ...
        if(gallery.length){
            ...
            slide_timer = undefined; // reset timer
    ...
    Shadowbox.close = function(){
        ...
        clearTimer();
        // reset slideshow delay variable for next time
        slide_delay = 0;
     ...
     * @static
     */
    Shadowbox.raise = function(error){
        if(options.handleException && typeof options.handleException == 'function'){
            options.handleException(error);
        }else{
            throw error;
        }
    };
